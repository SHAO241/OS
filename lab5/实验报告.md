...
```c
/**
 * copy_range - 将父进程的一段内存区域复制到子进程
 * @to:    子进程的页目录表基地址
 * @from:  父进程的页目录表基地址
 * @start: 要复制的起始虚拟地址（必须页对齐）
 * @end:   要复制的结束虚拟地址（必须页对齐，不包含）
 * @share: 是否共享（用于 COW，本实验中为 false）
 * 
 * 调用链：do_fork -> copy_mm -> dup_mmap -> copy_range
 * 
 * 功能：遍历 [start, end) 范围内的每一页，将父进程的物理页内容
 *       复制到子进程新分配的物理页中，并建立子进程的页表映射
 */
int copy_range(pde_t *to, pde_t *from, uintptr_t start, uintptr_t end,
               bool share)
{
    // 断言检查：起始和结束地址必须页对齐（4KB 边界）
    ...
    do
    {
        ...
        //   - start: 当前虚拟地址
        //   - 0: 不创建新页表（只查找）
        // 返回：指向页表项的指针，如果页表不存在则返回 NULL
        pte_t *ptep = get_pte(from, start, 0), *nptep;
        
        if (ptep == NULL)
        {
            // 父进程在该地址范围没有页表
            // 跳过整个页表覆盖的范围（PTSIZE = 2MB in Sv39）
            // ROUNDDOWN 向下对齐到页表边界，然后跳到下一个页表
            start = ROUNDDOWN(start + PTSIZE, PTSIZE);
            continue;
        }
        
        //============================================================
        // 第二步：检查页表项是否有效，获取/创建子进程的页表项
        //============================================================
        // PTE_V: Valid 位，表示该页表项有效（页面已映射）
        if (*ptep & PTE_V)
        {
            // 为子进程获取或创建页表项
            // get_pte(to, start, 1)：
            ...
            {
                return -E_NO_MEM;  // 内存不足，无法创建页表
            }
            
            // 获取父进程页表项的权限位
            // PTE_USER = PTE_U | PTE_R | PTE_W | PTE_X 等用户权限位
            uint32_t perm = (*ptep & PTE_USER);
            
            //============================================================
            // 第三步：获取父进程的物理页
            //============================================================
            // pte2page: 从页表项中提取物理页号，转换为 Page 结构体指针
            // 页表项格式：| PPN (物理页号) | RSW | D | A | G | U | X | W | R | V |
            ...
            //============================================================
            // 第五步：复制页面内容（LAB5 练习2 填写部分）
            //============================================================
            /* LAB5:EXERCISE2 2312991 */
            
            // (1) 获取父进程物理页的内核虚拟地址
            // page2kva: Page 结构体 -> 内核虚拟地址
            // 因为物理内存在内核中是线性映射的，所以可以直接访问
            ...
            // (4) 在子进程页表中建立映射
            // page_insert: 将物理页 npage 映射到虚拟地址 start
            //   - to: 子进程页目录
            //   - npage: 要映射的物理页
            //   - start: 虚拟地址
            //   - perm: 权限（与父进程相同）
...
│                                                                 │
│  【关键步骤】                                                   │
│  1. get_pte(from, addr, 0) - 获取父进程页表项                  │
│  2. pte2page(*ptep) - 获取父进程物理页                         │
│  3. alloc_page() - 为子进程分配新物理页                        │
│  4. page2kva() - 获取内核虚拟地址以便访问                      │
│  5. memcpy() - 复制 4KB 内容                                   │
│  6. page_insert(to, npage, addr, perm) - 建立子进程映射        │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
| 函数 | 作用 | 参数说明 |
|------|------|----------|
| `get_pte(pgdir, va, create)` | 获取页表项指针 | create=1 时自动创建页表 |
| `pte2page(pte)` | 页表项 → Page 结构体 | 提取 PPN 并转换 |
| `page2kva(page)` | Page → 内核虚拟地址 | 用于 memcpy 访问 |
| `alloc_page()` | 分配一个物理页 | 返回 Page 结构体指针 |
| `page_insert(pgdir, page, va, perm)` | 建立页表映射 | 设置 PTE 并刷新 TLB |
...
int do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf)
{
    ...
    // 设置父进程指针
    // Lab5 新增：建立父子关系，用于 wait/exit 机制
    proc->parent = current;
    
    // Lab5 新增：确保父进程不在等待状态
    // 如果父进程正在 wait，则不能 fork
    ...
    //   - 为真：共享内存空间（用于内核线程或创建线程）
    //   - 为假：复制内存空间（用于 fork 创建进程）
    if (copy_mm(clone_flags, proc) != 0) {
        goto bad_fork_cleanup_kstack;
    }
    
    //================================================================
    // 步骤 4: 设置 trapframe 和上下文
    //================================================================
    // copy_thread 完成以下工作：
    // - 在内核栈顶设置 trapframe
    // - 设置子进程返回值 a0 = 0（区分父子进程）
    // - 设置 context.ra = forkret（子进程第一次运行的入口）
    // - 设置 context.sp 指向 trapframe
    copy_thread(proc, stack, tf);
    
    //================================================================
    // 步骤 5: 将进程加入系统管理（关中断保护）
    //================================================================
    bool intr_flag;
    local_intr_save(intr_flag);  // 关中断，保证原子操作
    {
        proc->pid = get_pid();   // 分配唯一的 PID
        hash_proc(proc);         // 加入 hash 表（用于快速查找）
        set_links(proc);         // Lab5: 设置进程家族关系链接
    }
...
static void set_links(struct proc_struct *proc)
{
    ...
    // 2. 设置进程家族关系
    proc->yptr = NULL;  // younger sibling pointer 初始化为空
    
    // 3. 将新进程插入父进程的子进程链表头部
    if ((proc->optr = proc->parent->cptr) != NULL)
    {
        // 如果父进程已有子进程，设置原来的第一个子进程的 yptr
        proc->optr->yptr = proc;
    }
    proc->parent->cptr = proc;  // 父进程的 cptr 指向新子进程
...
│    NULL  ←──────────────┘  ←───────────────┘               │
│                                                             │
│  cptr: child pointer  - 指向第一个（最新的）子进程           │
│  optr: older sibling  - 指向更老的兄弟进程                  │
│  yptr: younger sibling - 指向更年轻的兄弟进程               │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

#### `copy_thread` - 设置子进程的执行环境

```c
static void copy_thread(struct proc_struct *proc, uintptr_t esp, struct trapframe *tf)
{
    // 1. 在内核栈顶分配 trapframe 空间
    proc->tf = (struct trapframe *)(proc->kstack + KSTACKSIZE) - 1;
    
    // 2. 复制父进程的 trapframe
    *(proc->tf) = *tf;

    // 3. 设置子进程返回值为 0（fork 的"调用一次，返回两次"）
    proc->tf->gpr.a0 = 0;
    
    // 4. 设置栈指针
    // esp=0 时是内核线程，使用 trapframe 地址作为栈指针
    // esp!=0 时是用户进程，使用传入的用户栈指针
    proc->tf->gpr.sp = (esp == 0) ? (uintptr_t)proc->tf : esp;

    // 5. 设置上下文，使子进程第一次被调度时从 forkret 开始执行
    proc->context.ra = (uintptr_t)forkret;  // 返回地址
    proc->context.sp = (uintptr_t)(proc->tf);  // 栈指针指向 trapframe
}
```
...
└───────────────────────────────────────────────────────────┘
        ...
        ▼
┌───────────────────────────────────────────────────────────┐
│  步骤 2: setup_kstack(proc)                               │
│  ├── 分配 KSTACKPAGE 页作为内核栈                          │
│  └── proc->kstack = 内核栈基地址                           │
└───────────────────────────────────────────────────────────┘
        │
        ▼
┌───────────────────────────────────────────────────────────┐
│  步骤 3: copy_mm(clone_flags, proc)                       │
│  ├── CLONE_VM 设置？                                      │
│  │   ├── 是：共享父进程的 mm（内核线程/线程）              │
│  │   └── 否：复制父进程的 mm（fork 进程）                  │
│  │       ├── mm_create() 创建新的 mm_struct               │
│  │       ├── setup_pgdir() 创建页目录                     │
│  │       └── dup_mmap() → copy_range() 复制内存内容       │
└───────────────────────────────────────────────────────────┘
        │
        ▼
┌───────────────────────────────────────────────────────────┐
│  步骤 4: copy_thread(proc, stack, tf)                     │
│  ├── 在内核栈顶设置 trapframe                              │
...
└───────────────────────────────────────────────────────────┘
        ...
        ▼
┌───────────────────────────────────────────────────────────┐
│  步骤 6: wakeup_proc(proc)                                │
│  └── proc->state = PROC_RUNNABLE                          │
└───────────────────────────────────────────────────────────┘
        │
        ▼
┌───────────────────────────────────────────────────────────┐
│  步骤 7: return proc->pid                                 │
│  └── 父进程返回子进程 PID                                  │
└───────────────────────────────────────────────────────────┘
```
...
proc->context:
  - ra = forkret  (第一次被调度时执行的函数)
  - sp = proc->tf (指向 trapframe)
```

### Lab4 与 Lab5 的 `do_fork` 区别

| 方面 | Lab4 | Lab5 |
|------|------|------|
| 父子关系 | 简单设置 `parent` | 需要检查 `wait_state`，使用 `set_links` |
| 进程链表 | 直接 `list_add` | 通过 `set_links` 维护家族关系 |
| 兄弟指针 | 无 | `optr`、`yptr` 构成兄弟链表 |
| `wait_state` | 无 | 初始化为 0，用于 wait/exit 机制 |
