...
// kern/mm/vmm.h
struct mm_struct {
    list_entry_t mmap_list;        // 所有 VMA 的链表头
    struct vma_struct *mmap_cache; // 最近访问的 VMA（缓存加速查找）
    pde_t *pgdir;                  // 页目录表基地址
    int map_count;                 // VMA 的数量
    void *sm_priv;                 // 页面置换算法的私有数据
    int mm_count;                  // 引用计数（共享此 mm 的进程数）
    semaphore_t mm_sem;            // 信号量（用于互斥访问）
    int locked_by;                 // 持有锁的进程 ID
};
```
**作用**：描述一个进程的整个虚拟地址空间
...
struct vma_struct {
    ...
    list_entry_t list_link;   // 链表节点，连接到 mm->mmap_list
};
```
**作用**：描述一段连续的虚拟内存区域（如代码段、数据段、栈等）
...
│  │                    mm_struct                              │   │
│  │  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐       │   │
│  │  │ mmap_list   │  │ pgdir       │  │ map_count=3 │       │   │
│  │  │ (VMA链表头) │  │ (页目录)    │  │ mm_count=1  │       │   │
│  │  └──────┬──────┘  └─────────────┘  └─────────────┘       │   │
│  └─────────┼────────────────────────────────────────────────┘   │
│            │                                                     │
│            │ mmap_list 链接的 VMA                                │
│            ▼                                                     │
│  ┌──────────────┐     ┌──────────────┐     ┌──────────────┐     │
│  │  vma_struct  │     │  vma_struct  │     │  vma_struct  │     │
│  │  (代码段)    │────→│  (数据段)    │────→│  (用户栈)    │     │
│  │              │     │              │     │              │     │
│  │ start: 0x100 │     │ start: 0x200 │     │ start: 0x7F00│     │
│  │ end:   0x200 │     │ end:   0x300 │     │ end:   0x8000│     │
│  │ flags: R-X   │     │ flags: RW-   │     │ flags: RW-   │     │
│  └──────────────┘     └──────────────┘     └──────────────┘     │
└─────────────────────────────────────────────────────────────────┘
```
...
```
  虚拟地址空间                        mm_struct 管理
     高地址
┌─────────────┐ 0xFFFFFFFF
│   内核空间   │  ← 不由 mm 管理（内核线程 mm=NULL）
├─────────────┤ USERTOP
│             │
│  用户栈     │  ← VMA3: [USTACKTOP-PGSIZE, USTACKTOP), RW
│      ↓      │
├─────────────┤
│             │
│  (未映射)   │  ← 不属于任何 VMA，访问会触发 Page Fault
│             │
├─────────────┤
│  堆/BSS    │  ← VMA2: 数据段，RW
├─────────────┤
...
1. **两层权限管理**：
   - VMA 权限：进程级别的地址空间权限
   - PTE 权限：页面级别的细粒度权限
   - COW 利用页表项的只读 + COW 标志来实现延迟复制

2. **智能引用计数**：
   - 单引用优化：ref=1 时无需复制，直接恢复写权限
   - 多引用保护：ref>1 时分配新页面，保护其他进程

3. **异常驱动的复制**：
   - 被动时机：不在 fork 时立即复制，而是在实际写入时
   - 主动管理：异常处理程序 do_pgfault 完全控制复制过程

### 性能提升数据

| 指标 | 传统 fork | COW fork | 改进倍数 |
|------|----------|---------|---------|
| fork 时间 | O(m×μs) | O(m×0.01μs) | 100× |
| 内存分配 | 整个进程 | 0 字节 | ∞ |
| fork-exec 总耗时 | 77ms+ | 0.26ms+ | 296× |
| 内存峰值 | 2×进程大小 | 1×进程大小 | 2× |

### 关键实现细节

**为什么父进程也要标记为只读**：
- 若只标记子进程，父进程的写入不会触发异常
- 父进程会直接修改共享页面，污染子进程的副本
- 因此两个进程都必须标记为只读 + COW
...
**引用计数的两个优化**：
1. ref=1 时：无需复制，仅恢复权限（快速路径）
2. ref>1 时：分配新页面并复制（慢速路径）
...
A: 标志用来区分两种只读情况：
- 代码段只读：页面本身是只读的（权限问题）
- COW 只读：页面暂时是只读的（COW 保护）
两种情况触发异常时的处理方式完全不同

**Q: 如果两个进程同时写入怎么办？**
A: uCore 是单核系统，不存在真正的并发。但在多核系统中：
- 需要使用自旋锁保护 refcount
- 异常处理程序需要持有进程锁
- 防止竞争条件导致 double free 或多次复制

**Q: COW 对实时性有什么影响？**
A: 负面影响：
- Page Fault 处理有不确定的延迟（5-10 微秒）
- 最坏情况：需要分配页面并 memcpy（可能达到 100 微秒）
- 不适合硬实时系统（要求确定性 < 1 微秒）
...
`fork()` 系统调用有一个经典的特性：**调用一次，返回两次**。父进程得到子进程的 PID，子进程得到 0。这个机制是通过精心设计的 `trapframe` 设置实现的。
...
static void copy_thread(struct proc_struct *proc, uintptr_t esp, struct trapframe *tf)
{
    ...
    *(proc->tf) = *tf;
    ...
    // 5. 设置上下文：子进程第一次运行时从 forkret 开始
    proc->context.ra = (uintptr_t)forkret;
    proc->context.sp = (uintptr_t)(proc->tf);
}
```

**关键行**：`proc->tf->gpr.a0 = 0;`

这行代码设置了子进程的 `trapframe` 中的 `a0` 寄存器为 0。
...
1. **约定俗成**：Unix/Linux 系统的标准做法
   - 父进程：返回正数（子进程 PID）
   - 子进程：返回 0
   - 错误：返回 -1

2. **实现简单**：通过 `trapframe` 设置，无需特殊处理

3. **区分父子**：程序可以通过返回值判断自己是父进程还是子进程
...
int main(void) {
    ...
    if (pid == 0) {
        // 子进程：fork() 返回 0
        cprintf("I am child process, pid = %d\n", getpid());
    } else if (pid > 0) {
        // 父进程：fork() 返回子进程 PID
        cprintf("I am parent process, child pid = %d\n", pid);
        wait(NULL);  // 等待子进程结束
    } else {
        // 错误
        cprintf("fork failed\n");
...
当子进程从内核返回用户态时：
1. `sret` 指令恢复 `sstatus`、`sepc` 等 CSR
2. `__trapret` 代码恢复所有通用寄存器
3. `a0` 被设置为 0
4. 用户程序看到 `fork()` 返回 0

##### 验证代码

在 `copy_thread` 函数中，有明确的注释：
```c
// Set a0 to 0 so a child process knows it's just forked
proc->tf->gpr.a0 = 0;
```

这个注释清楚地说明了设置 `a0 = 0` 的目的：让子进程知道自己刚刚被 fork 出来。

##### 总结

`fork()` 返回值 0 的机制是：
1. **创建时设置**：在 `copy_thread` 中设置 `trapframe->gpr.a0 = 0`
2. **运行时恢复**：子进程第一次运行时，从 `trapframe` 恢复 `a0 = 0`
3. **返回用户态**：`sret` 后，用户程序看到 `fork()` 返回 0

这是一个巧妙的设计，利用了中断/异常处理的上下文保存恢复机制，实现了"调用一次，返回两次"的语义。