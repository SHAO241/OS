...
## 算法核心思想
### 每个进程维护一个 stride
    每次选择 stride 最小的进程运行，运行后更新 stride：stride += BIG_STRIDE / priority
 优先级越高（priority 越大），stride 增量越小，更容易被选中

### 为什么时间片分配与优先级成正比？
    设进程 A 的优先级为 p_A，进程 B 的优先级为p_B。，进程 A 每次运行后 stride 增加：BIG_STRIDE / p_A
    进程 B 每次运行后 stride 增加：BIG_STRIDE / p_B

经过足够多的时间片后，两个进程的 stride 应该接近相等（否则会一直选择 stride 更小的进程）。设进程 A 运行了 n_A 次，进程 B 运行了 n_B 次：

    n_A * (BIG_STRIDE / p_A) ≈ n_B * (BIG_STRIDE / p_B)
简化后：
