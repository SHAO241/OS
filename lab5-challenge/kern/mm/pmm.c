               ...
               bool share)
{
    ...
    do
    {
        ...
        if (*ptep & PTE_V)
        {
            ...
            /* LAB5 CHALLENGE: Copy-on-Write Implementation
             * When share=1, use COW mechanism:
             * - Don't allocate new page, share the same physical page
             * - Clear write permission, set COW flag
             * - Increase page reference count
             */
            if (share)
            {
                // COW: Share the same physical page between parent and child
                // Remove write permission and set COW flag for both processes
                perm = (perm & ~PTE_W) | PTE_COW;
                
                // Update parent's PTE: remove write, add COW flag
                *ptep = pte_create(page2ppn(page), perm);
                
                // Map the same page to child with COW flag
                page_ref_inc(page);
                *nptep = pte_create(page2ppn(page), perm);
                
                // Invalidate TLB for both parent and child
