...
int dup_mmap(struct mm_struct *to, struct mm_struct *from)
{
    ...
    while ((le = list_prev(le)) != list)
    {
        ...
        // Use COW mechanism: share=1 enables Copy-on-Write
        bool share = 1;
 ...
 * 
 * CALL GRAPH: exception_handler --> do_pgfault
 *
 * This function handles Copy-on-Write page faults. When a process tries to
 * write to a COW page, a Store/AMO Page Fault is triggered.
 */
int do_pgfault(struct mm_struct *mm, uint32_t error_code, uintptr_t addr)
{
    int ret = -E_INVAL;
    ...
    {
        // Page not present - this should not happen for COW
    ...
    if (!(*ptep & PTE_COW))
    {
        // Not a COW page, this is a real access violation
        cprintf("do_pgfault: not a COW page at 0x%x\n", addr);
        goto failed;
    }
    
    // Handle COW: allocate a new page and copy content
    struct Page *old_page = pte2page(*ptep);
    struct Page *new_page = NULL;
    
    // Check if we are the only reference (ref count == 1)
    ...
    else
    {
        ...
        // Build new page table entry with write permission
        uint32_t perm = (*ptep & PTE_USER) | PTE_W;
        perm &= ~PTE_COW;  // Clear COW flag
        ...
        // Use page_insert which will handle the reference counting
