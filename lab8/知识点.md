...
└──────────────────────────────┘   │ │
        ...
        └──────────────┬───────────┘
            ...
            │ status: FD_OPENED            │
            │ fd: 3                        │
            │ pos: 1024 ← 共享！父子都是1024│  ← 关键：父子进程共享 pos
            │ node: ───────────────────────┼─┐
            │ open_count: 2 ← 从1变成2！   │ │  ← fork 后增加计数
            └──────────────────────────────┘ │
                                             │
                ┌────────────────────────────┘
                ▼
...
③ struct file：父子进程共享同一个（共享！）
   - 父进程 fd_array[3] 和子进程 fd_array[3] 指向同一个 file
   - open_count 从 1 增加到 2
   - pos 是共享的！父进程 read 后 pos=2048，子进程看到的也是 2048

④ struct inode：全局唯一（共享）
   - ref_count 和 open_count 不变（因为共享同一个 file）


============ 实际效果 ============

父进程执行：
read(3, buf, 100);  // pos: 1024 → 1124

子进程立即看到：
// pos 已经变成 1124 了！
read(3, buf2, 100); // pos: 1124 → 1224

这就是为什么 shell 的管道能正常工作：
ls | grep ".txt"
    ↑       ↑
  父进程  子进程
父进程写到 stdout (fd=1, pos=100)
子进程从 stdin (fd=0) 读，能接着读（共享 pos）
```
...
│  ├────────────────────────────┤    ├─────────────────────────────┤  │
│  │ size: 2048 (文件大小)      │    │ vop_magic: 0x8c4ba476       │  │
│  │ blocks: 10 (占用数据块数)  │    │ vop_open: sfs_open_impl     │  │
│  │ direct[0]: 100 (块号)      │    │ vop_close: sfs_close_impl   │  │
│  │ direct[1]: 101             │    │ vop_read: sfs_read_impl ────┼─┐│
│  │ direct[2]: 102             │    │ vop_write: sfs_write_impl   │ ││
│  │ ...                        │    │ vop_fstat: sfs_fstat_impl   │ ││
│  │ indirect: 200 (间接块)     │    │ vop_truncate: ...           │ ││
│  └────────────────────────────┘    │ ...                         │ ││
│                 │                  └─────────────────────────────┘ ││
│                 │                                                  ││
└─────────────────┼──────────────────────────────────────────────────┼┘
...
│  ├──────────────────────────────────────────────┤                 ││
│  │ 块 0: 超级块 (superblock)                    │                 ││
│  │ 块 1-9: inode 位图 & 数据块位图               │                 ││
│  │ 块 10-99: inode 数组                         │   ◄─────────────┘│
│  │   [5]: struct sfs_disk_inode (test.txt)      │   VFS 调用       │
│  │        - size: 2048                          │   vop_read()     │
│  │        - direct[0]: 100                      │       │          │
│  │        - direct[1]: 101                      │       │          │
│  │        - direct[2]: 102                      │       ▼          │
│  │        - ...                                 │   sfs_read_impl()│
│  │ 块 100: 数据块 (test.txt 的第 1 页)          │       │          │
│  │         [实际文件内容: "Hello World..."]      │   ◄───┘          │
│  │ 块 101: 数据块 (test.txt 的第 2 页)          │   读取磁盘块     │
│  │ 块 102: 数据块 (test.txt 的第 3 页)          │                  │
│  │ ...                                          │                  │
│  └──────────────────────────────────────────────┘                  │
│                                                                    │
└────────────────────────────────────────────────────────────────────┘
